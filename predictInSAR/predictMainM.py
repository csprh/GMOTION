...
for ii in range(0,2000):
    ...
    plotPredictions(values, s, "LSTM_M: RMSE = " +str(rmseLSTMM), y_hatLSTMM, "cyan", 0)
    ...
    np.save('M-LSTMM.npy', rmseLSTMMArray)
    np.save('M-LSTM1.npy', rmseLSTM1Array)
    np.save('M-LSTM6.npy', rmseLSTM6Array)
    np.save('M-LSTM5p.npy', rmseLSTM5pArray)
    np.save('M-Sarima.npy', rmseSarimaArray)
    np.save('M-Sinu.npy', rmseSinArray)

    plt.legend(loc='best')
    #plt.show()
    thisfig.savefig("Pred-M-"+str(chooseSeq)+".pdf", bbox_inches='tight')
    plt.close(); print('\n')
